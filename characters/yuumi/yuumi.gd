...
func turn_towards(target_direction: Vector3, max_angular_speed: float, delta):
	...
	var local_target_direction: Vector3 = $Yuumi.to_local($Yuumi.global_transform.origin + target_direction)
	var angle_to_target = atan2(-local_target_direction.x, -local_target_direction.z)
	var current_rotation: Quaternion = $Yuumi.global_transform.basis.get_rotation_quat()
	...
	if abs(angle_to_target) >= abs(max_rotation_angle): # I feel like there should be a bug on this line, but I can't see it happening if it is.
		$Yuumi.global_transform.basis = Basis(current_rotation * max_rotation)
	else:
		$Yuumi.global_transform.basis = Basis(current_rotation * Quaternion(Vector3.UP, angle_to_target))
